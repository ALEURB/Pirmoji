...
# Prepare data for modeling stage:
# - create new variables,
# - revalue existing variable
# - treat missing values.
# 
# Let’s start with missing values.
# We saw Product_Category_2 and Product_Category_3 had a lot of missing values.
# To me, this suggests a hidden trend which can be mapped by creating a new variable.
# So, we’ll create a new variable which will capture NAs as 1 and non-NAs as 0
# in the variables Product_Category_2 and Product_Category_3.

#create a new variable for missing values
# d[,Product_Category_2_NA := ifelse(sapply(d$Product_Category_2, is.na) == TRUE,1,0)]
# d[,Product_Category_3_NA := ifelse(sapply(d$Product_Category_3, is.na) == TRUE,1,0)]
#impute missing values
# d[,Product_Category_2 := ifelse(is.na(Product_Category_2) == TRUE, "-999",  Product_Category_2)]
# d[,Product_Category_3 := ifelse(is.na(Product_Category_3) == TRUE, "-999",  Product_Category_3)]

## Before proceeding to feature engineering, lastly,
# we’ll revalue variable levels as inferred from our univariate analysis.
#set column level
# levels(d$Stay_In_Current_City_Years)[levels(d$Stay_In_Current_City_Years) ==  "4+"] <- "4"

#set column level
# levels(d$Stay_In_Current_City_Years)[levels(d$Stay_In_Current_City_Years) ==  "4+"] <- "4"

#recoding age groups
# levels(d$Age)[levels(d$Age) == "0-17"] <- 0
# levels(d$Age)[levels(d$Age) == "18-25"] <- 1
# levels(d$Age)[levels(d$Age) == "26-35"] <- 2
# levels(d$Age)[levels(d$Age) == "36-45"] <- 3
# levels(d$Age)[levels(d$Age) == "46-50"] <- 4
# levels(d$Age)[levels(d$Age) == "51-55"] <- 5
# levels(d$Age)[levels(d$Age) == "55+"] <- 6
